...
/*1.filterType方法的返回值为过滤器的类型，过滤器的类型决定了过滤器在哪个生命周期执行，
pre表示在路由之前执行过滤器，其他可选值还有post、error、route和static，
当然也可以自定义。
...
public class PermisFilter extends ZuulFilter {
    ...
    public Object run() {
        ...
        if (name==null||!name.equals("Zhangmingyu")){
